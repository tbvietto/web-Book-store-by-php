...
<div style="float: right; margin-top: 30px; margin-bottom: 100px; width: 75%; margin-right: 25px">
    ...
    <!--SubCategories-->
    <div style="background-color: white; padding: 10px; margin-bottom: 15px; text-align: center; font-size: 20px; color: green;">-- <?php echo $_GET['cat_nm']; ?> --</div>

    <div style="background-color: white;">

        <?php
        foreach ($subcat_book as $value) {
            echo '<div>
                <div class="cardBook">
                    <div class="card">
        ...
        <div style="text-align: center; padding: 15px; clear: both">
            ...
            if ($current_page > 1 && $total_page > 1){
                echo '<a href="index.php?pageSub='.($current_page-1).'&cat_nm=' . $_GET['cat_nm'] . '&cat=' . $_GET['cat'] . '">Prev</a> | ';
            }
            for ($i = 1; $i <= $total_page; $i++){
                if ($i == $current_page){
                    ...
                    echo '<a href="index.php?pageSub='.$i.'&cat_nm=' . $_GET['cat_nm'] . '&cat=' . $_GET['cat'] . '">'.$i.'</a> | ';
                }
            }
            if ($current_page < $total_page && $total_page > 1){
                echo '<a href="index.php?pageSub='.($current_page+1).'&cat_nm=' . $_GET['cat_nm'] . '&cat=' . $_GET['cat'] . '">Next</a>';
            }
            ?>
        </div>
